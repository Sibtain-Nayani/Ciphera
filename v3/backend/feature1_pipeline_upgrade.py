"""
Ciphera V3 — Feature 1: Detection Pipeline v3.5
=================================================
Changes over v3.2:
  - Falls back gracefully: en_core_web_trf → en_core_web_lg (no crash)
  - Tighter DOB regex: requires day ≤ 31 and month ≤ 12, rejects version numbers
  - Added: UPI ID, Bank Account Number, Driving Licence (India)
  - Better Indian name detection via context keyword boosting
  - Aadhaar false positive fix: rejects 12-digit numbers that look like phone/order IDs
  - Removes DATE_OF_BIRTH matches that overlap with phone/aadhaar detections
  - Faster: disabled parser+lemmatizer in spaCy for all models
"""
...
class OCRCleaner:
    ...
    def clean(self,text:str)->str:
        for p,r in self.SUBS: text=p.sub(r,text)
        return text.strip()
...
class DetectionSource(str,Enum):
    REGEX="regex"; PRESIDIO="presidio"; SPACY="spacy"; MERGED="merged"
...
class DetectedEntity:
    ...
    def to_dict(self)->dict:
        return {"start":self.start,"end":self.end,"entity_type":self.entity_type,
                "text":self.text,"score":round(self.score,4),"source":self.source.value,
                "context":self.context,"merged_from":[s.value for s in self.merged_from]}

# ── Regex Stage ───────────────────────────────────────────────────────────────
_MONTHS=(r"(?:January|February|March|April|May|June|July|August|September|"
         r"October|November|December|Jan|Feb|Mar|Apr|Jun|Jul|Aug|Sep|Oct|Nov|Dec)")

# Strict DOB: validates day<=31 month<=12 before matching
# Uses lookahead/lookbehind to avoid version numbers like 1.2.3
_DOB_PATTERNS=[
    # DD/MM/YYYY or DD-MM-YYYY — validates ranges
    r"\b(?:0?[1-9]|[12]\d|3[01])[\/\-\. ](?:0?[1-9]|1[0-2])[\/\-\. ](?:19|20)\d{2}\b",
    # YYYY-MM-DD ISO — only valid centuries
...
# Pattern to detect things that look like version numbers (reject these as DOB)
...
class RegexStage:
    PATTERNS=[
        ...
        # Phone — +91 or bare 10-digit starting 6-9
        ...
        (r"\b[a-zA-Z0-9.\-_]{2,256}@[a-zA-Z]{2,64}\b", "UPI_ID",          0.82),
        # Bank account — 9 to 18 digits, requires context keyword nearby
        (r"\b\d{9,18}\b",                                "BANK_ACCOUNT",    0.60),
        # Indian Driving Licence — StateCode-YY-NNNNNNN
        (r"\b[A-Z]{2}[\s\-]?\d{2}[\s\-]?\d{4}[\s\-]?\d{7}\b","DRIVING_LICENCE",0.78),
        # Vehicle reg
        (r"\b[A-Z]{2}[\s\-]?\d{1,2}[\s\-]?[A-Z]{1,3}[\s\-]?\d{4}\b","IN_VEHICLE_REG",0.72),
        # IPv4
        (r"\b(?:\d{1,3}\.){3}\d{1,3}\b",                "IP_ADDRESS",      0.82),
        # URL
        (r"https?://[^\s\"'<>]+",                        "URL",             0.90),
        # Credit card
        (r"\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12})\b","CREDIT_CARD",0.88),
    ]
    ...
    def analyze(self,text:str)->list[DetectedEntity]:
        ...
        for pattern,entity_type,base_score in self._compiled:
            for m in pattern.finditer(text):
                raw=m.group()
                if raw.strip().lower() in SUPPRESSION_LIST: continue
                # Reject UPI matches that are actually email addresses
                if entity_type=="UPI_ID" and "." in raw.split("@")[0]: continue
                # Bank account: only emit if context has banking keywords
                if entity_type=="BANK_ACCOUNT":
                    ctx=_get_context(text,m.start(),m.end(),60).lower()
                    if not any(kw in ctx for kw in ["account","a/c","acc","bank","savings","current","neft","rtgs","imps"]):
                        continue
                score=self._validate(raw,entity_type,base_score)
                if score==0: continue
                results.append(DetectedEntity(
                    start=m.start(),end=m.end(),entity_type=entity_type,text=raw,
        ...
        for m in _DOB_RE.finditer(text):
            ...
            if _VERSION_RE.match(raw): continue
            results.append(DetectedEntity(
                start=m.start(),end=m.end(),entity_type="DATE_OF_BIRTH",text=raw,
                score=0.82,source=DetectionSource.REGEX,
                context=_get_context(text,m.start(),m.end()),type_locked=True,
            ))
    ...
    def _validate(value:str,entity_type:str,base_score:float)->float:
        if entity_type=="AADHAAR_NUMBER":
            ...
            # Reject if starts with 0 or 1 (invalid Aadhaar)
            if digits[0] in "01": return 0
            # Reject phone-like patterns (all same digit, sequential)
        ...
        if entity_type=="IP_ADDRESS":
            ...
            # Private IPs are less interesting but still PII
...
class PresidioStage:
    ...
    def _add_custom(self):
        for rec in [
            ...
            PatternRecognizer("UPI_ID",patterns=[
                Pattern("UPI",r"\b[a-zA-Z0-9.\-_]{2,256}@[a-zA-Z]{2,64}\b",0.80),
    ...
    def analyze(self,text:str)->list[DetectedEntity]:
        ...
        for r in self.analyzer.analyze(text=text,entities=self.TARGET_ENTITIES,language="en"):
            ...
            if span.strip().lower() in SUPPRESSION_LIST: continue
            results.append(DetectedEntity(
...
# ── spaCy Stage ───────────────────────────────────────────────────────────────
class SpacyNERStage:
    LABEL_MAP={"PERSON":"PERSON","ORG":"ORGANIZATION","GPE":"LOCATION","LOC":"LOCATION","FAC":"LOCATION","DATE":"DATE_TIME","TIME":"DATE_TIME"}
    ...
    def analyze(self,text:str)->list[DetectedEntity]:
        ...
        doc=self.nlp(text)
        for ent in doc.ents:
            if ent.label_ in SPACY_IGNORE_LABELS: continue
            mapped=self.LABEL_MAP.get(ent.label_)
            if not mapped: continue
            if ent.text.strip().lower() in SUPPRESSION_LIST: continue
            # Reject 10-digit numbers as DATE_TIME (they're phone numbers)
            if mapped=="DATE_TIME" and re.match(r"^\d{10}$",ent.text.strip()): continue
            # Boost score for Indian names with title context
            score=0.72
            ctx=_get_context(text,ent.start_char,ent.end_char,40).lower()
            if mapped=="PERSON" and any(t in ctx for t in ["mr","mrs","ms","dr","shri","smt","kumari","sh.","smt."]):
                score=0.82
            results.append(DetectedEntity(
                start=ent.start_char,end=ent.end_char,entity_type=mapped,
                text=ent.text,score=score,source=DetectionSource.SPACY,
...
CONTEXT_BOOSTS=[
    ...
    ("DATE_OF_BIRTH",["dob","born","birth","date of birth","birthdate","d.o.b","जन्म"],+0.10),
    ("PERSON",["mr","mrs","ms","dr","shri","smt","name","applicant","candidate","signed","shri.","smt.","kumari"],+0.10),
    ("PHONE_NUMBER",["phone","mobile","cell","contact","tel","whatsapp","mob"],+0.10),
...
def merge_and_vote(candidates:list[DetectedEntity])->list[DetectedEntity]:
    ...
    cur=[candidates[0]]; end=candidates[0].end
    for entity in candidates[1:]:
        if entity.start<end:
            cur.append(entity); end=max(end,entity.end)
        else:
            groups.append(cur); cur=[entity]; end=entity.end
    groups.append(cur)
    ...
    for group in groups:
        ...
        wscore=sum(e.score*SOURCE_WEIGHTS.get(e.source.value,1.0) for e in group)/total_w
        if wscore<CONFIDENCE_THRESHOLD: continue
        floor=TYPE_FLOOR.get(elected_type,CONFIDENCE_THRESHOLD)
        if wscore<floor: continue
...
class DetectionPipeline:
    def __init__(self,use_transformer:bool=True):
        # Try transformer first, fall back gracefully
        ...
        disabled=[p for p in ["parser","attribute_ruler","lemmatizer"] if p in nlp.pipe_names]
        if disabled: nlp.disable_pipes(*disabled)

        self.regex_stage    = RegexStage()
        self.presidio_stage = PresidioStage(model)
        self.spacy_stage    = SpacyNERStage(nlp)
        logger.info("DetectionPipeline v3.5 ready (model=%s).",model)

    def run(self,text:str,threshold:float=CONFIDENCE_THRESHOLD,
            enabled_stages:Optional[list[str]]=None,clean_ocr:bool=False)->list[DetectedEntity]:
        stages=enabled_stages or ["regex","presidio","spacy"]
        if clean_ocr: text=ocr_cleaner.clean(text)
