    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div className="relative z-10 w-full max-w-3xl mx-4 bg-[#1A1A1A] border border-[#2A2A2A] rounded-2xl shadow-2xl flex flex-col max-h-[80vh]">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#2A2A2A] shrink-0">
                ...
                <button onClick={onClose} className="p-1.5 text-gray-500 hover:text-white hover:bg-[#2A2A2A] rounded-lg cursor-pointer transition-colors"><X className="w-4 h-4" /></button>
            </div>
...
export default function BatchPage() {
    ...
    const addFiles = (files: FileList | File[]) => {
        ...
        if (skipped > 0) useUiStore.getState().addToast(`${skipped} file(s) skipped — PDF & images require the Sanitize page`, 'warning');
            ...
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                <header className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 pb-4 border-b border-[#2A2A2A]">
                    <div>
                        <h1 className="text-xl font-semibold text-white flex items-center gap-2.5">
                            <Layers className="w-5 h-5 text-[#818CF8]" />Assembly Line
                        </h1>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        {doneCount > 0 && (
                            <button onClick={downloadAll} className="flex items-center gap-2 px-4 py-2 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 border border-emerald-500/30 rounded-xl text-sm font-medium transition-all cursor-pointer">
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                        <span className="text-xs text-gray-500 font-medium shrink-0">Export all as:</span>
                ...
                {jobs.length > 0 && (
                    <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#2A2A2A] bg-[#1A1A1A]">
                            ...
                            <div className="flex items-center gap-3">
                                {doneCount > 0 && <button onClick={clearDone} className="text-xs text-gray-500 hover:text-gray-300 cursor-pointer">Clear completed</button>}
                                <button onClick={clearAll} disabled={isRunning} className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-red-400 cursor-pointer disabled:opacity-30"><Trash2 className="w-3.5 h-3.5" /> Clear All</button>
                            </div>
                        </div>

                        <div className="divide-y divide-[#2A2A2A]/60">
                            {jobs.map(job => (
                                <div key={job.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-[#181818] transition-colors group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="text-[10px] text-gray-600 font-mono shrink-0">{(job.file.size / 1024).toFixed(1)} KB</span>
                                        </div>
                                        ...
                                        {job.status === 'done' && <p className="text-[11px] text-emerald-400 font-mono">{job.entities} entities redacted</p>}
                                        {job.status === 'error' && <p className="text-[11px] text-red-400">{job.error}</p>}
                                    </div>

                                    {/* Actions — right side */}
                                    <div className="flex items-center gap-2 shrink-0">
                                        <span className={`text-[11px] font-medium whitespace-nowrap ${STATUS_COLOR[job.status]}`}>{STATUS_LABEL[job.status]}</span>

                                        {job.status === 'done' && job.result && (
                                            <>
                                                <button onClick={() => setPreviewJob(job)}
                                                    className="flex items-center gap-1 px-2.5 py-1 text-[11px] text-gray-400 hover:text-white hover:bg-[#2A2A2A] rounded-lg transition-colors cursor-pointer border border-[#2A2A2A] hover:border-[#3A3A3A] whitespace-nowrap">
                                                    <Eye className="w-3 h-3" /> Preview
                                                </button>
                                                <button onClick={() => downloadOne(job)}
                                                    className="flex items-center gap-1 px-2.5 py-1 text-[11px] text-[#FFA500] hover:text-black hover:bg-[#FFA500] rounded-lg transition-all cursor-pointer border border-[#FFA500]/30 whitespace-nowrap">
                                        ...
                                        {job.status !== 'processing' && (
                                            <button onClick={() => removeJob(job.id)}
                                                className="p-1.5 text-gray-600 hover:text-red-400 hover:bg-[#2A2A2A] rounded-lg cursor-pointer opacity-0 group-hover:opacity-100 transition-all">
