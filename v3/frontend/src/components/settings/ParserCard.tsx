...
const PARSER_META: Record<RuleType, ParserMeta> = {
    ...
    email: {
        ...
        iconBgClass: 'bg-blue-500/10', iconBorderClass: 'border-blue-500/20', iconTextClass: 'text-blue-400',
    ...
    creditCard: {
        ...
        iconBgClass: 'bg-purple-500/10', iconBorderClass: 'border-purple-500/20', iconTextClass: 'text-purple-400',
        regex: '/(?:\\d{4}[-\\s]?){3}\\d{4}/',
        regexLabel: 'RegEx', actions: STANDARD_ACTIONS,
    },
    ssn: {
        ...
        iconBgClass: 'bg-gray-500/10', iconBorderClass: 'border-gray-500/20', iconTextClass: 'text-gray-400',
    ...
    names: {
        ...
        iconBgClass: 'bg-[#FFA500]/10', iconBorderClass: 'border-[#FFA500]/20', iconTextClass: 'text-[#FFA500]',
        regex: 'en_core_web_lg + Presidio',
        regexLabel: 'spaCy', actions: STANDARD_ACTIONS,
    },
    dob: {
        label: 'Date of Birth',
        icon: <CalendarDays className="w-5 h-5" />,
        iconBgClass: 'bg-red-500/10', iconBorderClass: 'border-red-500/20', iconTextClass: 'text-red-400',
        regex: 'DD/MM/YYYY · DD-MM-YYYY · DD.MM.YYYY · D Mon YYYY + 6 more',
        regexLabel: 'Multi-RegEx', actions: STANDARD_ACTIONS,
    },
    date: {
        label: 'General Dates',
        icon: <Calendar className="w-5 h-5" />,
        iconBgClass: 'bg-slate-500/10', iconBorderClass: 'border-slate-500/20', iconTextClass: 'text-slate-400',
        regex: 'spaCy DATE + NLP context',
        regexLabel: 'spaCy', actions: STANDARD_ACTIONS,
    },
    url: {
        label: 'URLs',
        icon: <Globe className="w-5 h-5" />,
        iconBgClass: 'bg-sky-500/10', iconBorderClass: 'border-sky-500/20', iconTextClass: 'text-sky-400',
        regex: '/https?:\\/\\/[^\\s"\'<>]+/',
        regexLabel: 'RegEx', actions: STANDARD_ACTIONS,
    },
    ip: {
        ...
        iconBgClass: 'bg-lime-500/10', iconBorderClass: 'border-lime-500/20', iconTextClass: 'text-lime-400',
        regex: '/(?:\\d{1,3}\\.){3}\\d{1,3}/',
        regexLabel: 'RegEx', actions: STANDARD_ACTIONS,
    },
    ...
    aadhaar: {
        ...
        iconBgClass: 'bg-orange-500/10', iconBorderClass: 'border-orange-500/20', iconTextClass: 'text-orange-400',
        regex: '/\\d{4}[\\s-]?\\d{4}[\\s-]?\\d{4}/ + Verhoeff CRC',
        regexLabel: 'RegEx+CRC', actions: STANDARD_ACTIONS,
    },
    pan: {
        label: 'PAN Number',
        icon: <BookKey className="w-5 h-5" />,
        iconBgClass: 'bg-yellow-500/10', iconBorderClass: 'border-yellow-500/20', iconTextClass: 'text-yellow-400',
    ...
    gst: {
        ...
        iconBgClass: 'bg-teal-500/10', iconBorderClass: 'border-teal-500/20', iconTextClass: 'text-teal-400',
    ...
    voterId: {
        ...
        iconBgClass: 'bg-pink-500/10', iconBorderClass: 'border-pink-500/20', iconTextClass: 'text-pink-400',
        regex: '/[A-Z]{3}[0-9]{7}/',
        regexLabel: 'RegEx', actions: STANDARD_ACTIONS,
    },
    passport: {
        ...
        iconBgClass: 'bg-indigo-500/10', iconBorderClass: 'border-indigo-500/20', iconTextClass: 'text-indigo-400',
...
export function ParserCard({ ruleKey }: { ruleKey: RuleType }) {
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(async () => {
            const singleRule = { [ruleKey]: config } as Record<RuleType, import('@/store/documentStore').RuleConfig>;
            const result = await redactionEngine.tokenize(testInput, singleRule, []);
            if (!result.failed) {
                const redacted = result.tokens.map(t => {
                    if (t.type === 'text') return t.value;
                    if (t.type === ruleKey)
                        return redactionEngine.getRedactionReplacement(t.type, t.value, config.action, []);
                    return t.value;
                }).join('');
    ...
    return (
        <article
            ...
            className={`group bg-card rounded-xl border border-border p-6 shadow-lg transition-all duration-300 animate-in fade-in hover:border-[#3B3B3B]/80 hover:shadow-xl ${!isActive ? 'opacity-50' : ''}`}
        >
            <div className="flex items-center justify-between mb-6">
                ...
                <ToggleSwitch id={`toggle-${ruleKey}`} checked={isActive} onChange={() => toggleRule(ruleKey)} />
            </div>
            ...
            <div className="space-y-5">
                ...
                <div className="pt-4 border-t border-[#3B3B3B]/50">
                    <div className="mb-4">
                        <label className="block text-xs font-medium text-muted-foreground mb-2 uppercase tracking-wider">Redaction Action</label>
                        <select disabled={!isActive} value={config?.action} onChange={(e) => setRuleAction(ruleKey, e.target.value as RedactionAction)}
                            className={`w-full rounded-lg px-4 py-2.5 text-sm border border-border appearance-none transition-all focus:outline-none ${isActive ? 'bg-input text-white cursor-pointer hover:border-[#FFA500]/50' : 'bg-background text-gray-500 cursor-not-allowed opacity-60'}`}>
                            {meta.actions.map((a) => <option key={a.value} value={a.value}>{a.label}</option>)}
                        </select>
                    </div>

                    <div className="bg-[#1A1A1A] rounded-xl border border-[#3B3B3B] p-3 focus-within:border-[#FFA500]/50 transition-colors">
                        <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 px-1">Live Output Sandbox</label>
                        <input type="text" placeholder={isActive ? "Type a test string here..." : "Rule disabled"} disabled={!isActive}
